...
var CvZbuf = /** @class */ (function () {
    ...
    CvZbuf.prototype.paint = function () {
        ...
        var baseR = this.obj.baseColorR || 200;
        var baseG = this.obj.baseColorG || 200;
        var baseB = this.obj.baseColorB || 200;
        for (var j = 0; j < nFaces; j++) {
            ...
            for (var i = 0; i < t.length; i++) {
                ...
                var nA = this.obj.vNormals[iA], nB = this.obj.vNormals[iB], nC = this.obj.vNormals[iC];
                var cAi = this.obj.colorCode(nA.x, nA.y, nA.z);
                var cBi = this.obj.colorCode(nB.x, nB.y, nB.z);
                var cCi = this.obj.colorCode(nC.x, nC.y, nC.z);
                if (isNaN(cAi))
                    cAi = 255;
                if (isNaN(cBi))
                    cBi = 255;
