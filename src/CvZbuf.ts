...
export class CvZbuf{
  ...
  paint(): void{
    ...
    let baseR = this.obj.baseColorR || 200;
    let baseG = this.obj.baseColorG || 200;
    let baseB = this.obj.baseColorB || 200;
    ...
    for (let j = 0; j < nFaces; j++) {  
      ...
      for (let i = 0; i < t.length; i++){  
        ...
        let nA = this.obj.vNormals[iA], nB = this.obj.vNormals[iB], nC = this.obj.vNormals[iC];
        let cAi = this.obj.colorCode(nA.x, nA.y, nA.z);
        let cBi = this.obj.colorCode(nB.x, nB.y, nB.z);
        let cCi = this.obj.colorCode(nC.x, nC.y, nC.z);
